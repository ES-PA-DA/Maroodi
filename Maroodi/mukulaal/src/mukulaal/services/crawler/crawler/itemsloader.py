...
def parse_unit(label:str)->str:
    ...
    unit_label = unit_label[-1].strip() if len(unit_label) > 2 else label.split(" ")[-1].strip()
...
def parse_amount(label:str)->str:
    ...
    amount_label = amount_label[-1].strip() if len(amount_label) > 2 else label.strip().split(" ")[-1].strip()
